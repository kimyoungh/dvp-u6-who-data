...
answer = {}
answer['Highest'] = 'United States of America'
answer['Lowest'] = 'Zimbabwe'
...
lanswer = {}
lanswer['Highest'] = 'Germany'
lanswer['Lowest'] = 'Zimbabwe'
...
gl = sns.FacetGrid(df, col='Country', col_wrap=3, size=4)
gl = gl.map(plt.scatter, 'GDP', 'LEABY').add_legend()
gl.set_ylabels("Life Expectancy by Country")
plt.savefig('gl.png')

g = sns.FacetGrid(df, col='Country', col_wrap=3, size=4)
g = g.map(plt.plot, 'Year', 'GDP', color='Red').add_legend()
plt.savefig('g.png')

gy = sns.FacetGrid(df, col='Country', col_wrap=3, size=4)
gy = gy.map(plt.plot, 'Year', 'LEABY', color='Green').add_legend()
gy.set_ylabels('Life Expectancy by Country')
